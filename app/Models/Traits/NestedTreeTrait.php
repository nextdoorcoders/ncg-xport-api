...
trait NestedTreeTrait
{
     ...
     * Constructor
     */
    public static function bootNestedTreeTrait(): void
    {
        ...
        if (static::hasGlobalScope(SoftDeletingScope::class)) {
            self::restoring(function (self $model) {
     ...
     *
     * @param self   $node     The node to be moved
     * @param self   $target   The target node to be moved relative to
     * @param string $position One of the following values: child, left, right
     * @return int
     */
    protected function performMove($node, $target, $position)
    {
        ...
        $wrappedId = 'CAST(' . $grammar->wrap($node->getKeyName()) . ' AS TEXT)'; // Only for pgsql
